...
| FCC 이상시점 추가 이상 목록 | `/appdata/hadoop/code/eads/PFB3/D1c_fcc/fcc_timefit/fail_fccdate_list.parquet` | `step_seq`, `eqp_ch`, `tkout_time`, `main_step`, `fcc_step_seq` |
...
| FCC 이상시점 all 배경 scatter | `/appdata/hadoop/code/eads/PFB3/D1c_fcc/fcc_timefit/U%{step_seq}/{met_step}_{item_id}/{latestDate}/all_fccdate_{eqp_ch}_U%{step_seq}.parquet` | `tkout_time`, `defect_value`, `wafer_id`, `lot_id`, `lot_wf`, `step_seq`, `eqp_ch`, `process_id`, `item_id`, `item_desc`, `ppid`, `ppid_right`, `final_decision` |
| FCC 이상시점 fail 중심치 이상 scatter | `/appdata/hadoop/code/eads/PFB3/D1c_fcc/fcc_timefit/U%{step_seq}/{met_step}_{item_id}/{latestDate}/fail_fccdate_{eqp_ch}_U%{step_seq}.parquet` | `tkout_time`, `defect_value`, `wafer_id`, `lot_id`, `lot_wf`, `step_seq`, `eqp_ch`, `process_id`, `item_id`, `final_decision` |
...
FCC 이상시점 추가 이상 목록은 FCC지수 연관 이상감지의 FCC 중심치 이상 목록에서 온 `main_seq`의 `%` 뒤 숫자 6자리를 `fail_fccdate_list.parquet.main_step`과 매칭하고, `met_seq`의 `_` 앞 숫자 6자리를 `fail_fccdate_list.parquet.fcc_step_seq`와 매칭해 같은 FCC 중심치 `main_step`/`met_step` 하위의 `eqp_ch`별 이상 건수를 집계한다. chart drawing은 매칭된 `fail_fccdate_list.parquet.step_seq`를 기준으로 `fcc_timefit/U%{step_seq}/{met_step}_{item_id}/{latestDate}` 아래 `all_fccdate_{eqp_ch}_U%{step_seq}.parquet`, `fail_fccdate_{eqp_ch}_U%{step_seq}.parquet`를 읽고, 해당 FCC 중심치 이상 `met_step`의 기존 FCC chart stack에 `FCC 이상시점 fail 중심치 이상 scatter` 섹션으로 추가한다.
