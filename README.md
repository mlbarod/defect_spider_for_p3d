...
FCC chart의 `main_step`과 `met_step`은 FCC MET 매핑 파일 값을 사용하며, `U%` prefix를 제거한 뒤 뒤의 여섯자리 step 값을 기준으로 경로를 만든다. `item_id`는 `met_step`이 `125433`이면 `26`, `13013`이면 `186`을 사용한다.
