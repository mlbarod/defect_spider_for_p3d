...
FCC chart의 `main_step`은 FCC 스탭 MET 매핑 파일 값을 사용하며, `U%` prefix를 제거한 뒤 뒤의 여섯자리 step 값을 기준으로 경로를 만든다. FCC 중심치 이상 목록의 `met_seq`는 `{met_step}_{item_id}` 형식이며, 앞의 여섯자리 숫자를 `met_step`, `_` 뒤 숫자를 `item_id`로 사용한다.
