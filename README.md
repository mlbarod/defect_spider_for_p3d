...
| FCC 이상시점 추가 이상 목록 | `/appdata/hadoop/code/eads/PFB3/D1c_fcc/fcc_timefit/fail_fccdate_list.parquet` | `step_seq`, `eqp_ch`, `tkout_time`, `main_step` |
...
FCC 이상시점 추가 이상 목록은 FCC지수 연관 이상감지에서 선택한 `main_step`과 `fail_fccdate_list.parquet.main_step`을 매칭해 같은 `main_step`의 `eqp_ch`별 이상 건수를 집계한다. 선택된 FCC 버튼의 `met_step`/`item_id`와 목록의 `eqp_ch`를 사용해 `fcc_timefit/U%{main_step}/{met_step}_{item_id}/{latestDate}` 아래 `all_fccdate_{eqp_ch}_U%{main_step}.parquet`, `fail_fccdate_{eqp_ch}_U%{main_step}.parquet`를 읽고, 기존 FCC chart stack에 `FCC 이상시점 fail 중심치 이상 scatter` 섹션으로 추가한다.
